"""
Investment Dashboard - Streamlit Application with DynamoDB CRUD Operations
"""
...
# ==================== VIEW ALL PAGE ====================
    ...
    try:
        ...
        if not investments:
            ...
            investments = sorted(
                investments, 
                key=lambda x: x.get('investment_date', ''), 
                reverse=True
            )
            ...
            for idx, inv in enumerate(investments, 1):
                with st.expander(
                    ...
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("✏️ Edit", key=f"edit_{inv.get('investment_id')}"):
                            st.session_state.selected_investment_id = inv.get('investment_id')
...
# ==================== UPDATE PAGE ====================
    ...
    try:
        ...
        if not investments:
            ...
            investment_options = {
                f"{inv.get('investment_date', 'N/A')} - ₹{float(inv.get('investment_amount', 0)):,.2f}": 
                inv.get('investment_id') 
                for inv in investments
            }
                    ...
                    with st.form("update_investment_form"):
                        ...
                        with col2:
                            inv_date_obj = datetime.strptime(
                                selected_investment.get('investment_date', ''), 
                                "%Y-%m-%d"
                            ).date()
        ...
        if not investments:
            ...
            if selected_display:
                ...
                if selected_investment:
                    ...
                    with col2:
                        st.metric("Date", selected_investment.get('investment_date', 'N/A'))
                    with col3:
                        st.metric(
                            "Annual Return %",
...
st.markdown(
    ...
    Investment Dashboard v1.0 | Built with Streamlit & AWS DynamoDB
    </div>
    """,
    unsafe_allow_html=True
)
